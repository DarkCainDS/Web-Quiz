...
import questions2 from './questions20';
...
export const Quiz2 = ({scenario}) => {
  ...
  const nextQuestion = (isCorrect,) => {
    ...
    if (forwardQuestion < questions2.length) {
  ...
  if (answerShown)
    return (
      <main className="quiz">
        <ResultQuiz score={score} question={questions2} scenario={scenario}/>
        <div className="container">
          <div className="question-count">
          <span> Question {currentQuestion + 1} of</span> {questions2.length}
          </div>
          <div className="question-text">
            {questions2[currentQuestion].questionTitle}
          </div>
          <div className='answer-section  answer-individual'>
            {
              questions2[currentQuestion].options.filter(
      ...
      <div className='container'>
        {showScore ? (
          <div className='score-section'>
            <span>You scored {score} out of {questions2.length}</span>
            ...
            <div className='question-section'>
              <div className='question-count'>
                ...
                <div className='next-question'>
                  {!buttonDisabled ? (<span className='timer'>Left time: {leftTime}</span>) : (<button className='quiz-button' onClick={() => { setLeftTime(60); setButtonDisabled(false); setCurrentQuestion(currentQuestion + 1); if (currentQuestion === questions2.length - 1) { setShowScore(true) } }}>Next Question</button>)}
                </div>
              </div>
              <div className='question-text'>{questions2[currentQuestion].questionTitle}</div>
            </div>
            <div className='answer-section'>
              {questions2[currentQuestion].options.map((respuesta,i) => (
